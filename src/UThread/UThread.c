...
HANDLE UtCreate (UT_FUNCTION Function, UT_ARGUMENT Argument, size_t Stack_Size, const char* Thread_Name) {
	...
	if (Stack_Size == NULL || Stack_Size < STACK_SIZE) //NULL?
...
HANDLE UtCreate (UT_FUNCTION Function, UT_ARGUMENT Argument) {
	...
	//
	Thread = (PUTHREAD) malloc(sizeof (UTHREAD));
	Thread->Stack = (PUCHAR) malloc(Thread->Stack_Size);
	_ASSERTE(Thread != NULL && Thread->Stack != NULL);

	//
	// Zero the stack for emotional confort.
	//
	memset(Thread->Stack, 0, Thread->Stack_Size);
	...
	Thread->ThreadContext = (PUTHREAD_CONTEXT) (Thread->Stack +
		Thread->Stack_Size -sizeof (UTHREAD_CONTEXT)-sizeof(ULONGLONG)*5);
